...
static void apply_balance(const AVL_Root root, _Node *this) {
    ...
    int balance;
    //_Node upper_node_after_rotate = this;
...
} /* end of static void apply_balance(const _Node) */

static Object Tree_DeleteElement(const AVL_Root root, _Node this, const Object info) {
    ...
    compare_result = cmp(info, this->info);
    
    if ((root->num_of_elements == 1) && (compare_result == 0)) {
        ret_info = this->info;
        free(this);
        root->childs = NULL;
        return ret_info;
    ...
    if (compare_result > 0)
        ret_info = Tree_DeleteElement(root, this->left, info);
    else if (compare_result < 0)
        ret_info = Tree_DeleteElement(root, this->right, info);
    else if ((this->left) && (this->right)) {
        _Node largest_node = Tree_FindLargestElement(this->left);        
        ret_info = this->info;
        this->info = largest_node->info;
        
        if (largest_node == this->left) 
            largest_node->parent->left = largest_node->left;
		    else             
            largest_node->parent->right = largest_node->left;
                
		    if (largest_node->left)
		        largest_node->left->parent = largest_node->parent;
                    
        setNodeHeight(largest_node->parent);        
        apply_balance(root, &(largest_node->parent));
        
        free(largest_node);                        
        return ret_info;
    } else {            
        which_node = (this->parent->left == this) ? LEFT_NODE : RIGHT_NODE;
        ret_info = this->info;
        if ((this->left == NULL) && (this->right == NULL)) {
                                
            if (which_node == LEFT_NODE) this->parent->left = NULL;
...
} /* end of static Object Tree_DeleteElement(const AVL_Root, _Node, const Object) */
...
static Object Tree_DeleteElement_v2(const AVL_Root root, const Object info) {
    ...
    if (to_be_deleted_node->left->right == NULL) {
        ...
        if (to_be_deleted_node->left->left == NULL) {
            ...
            else node->right = to_be_deleted_node->left;  
            free(to_be_deleted_node);         
        }      
        Tree_Restruct(root, node);
        return ret_info; 
...
Object AVL_Tree_DeleteElement(const AVL_Root this, Object info) {    
    ...
    //ret_info = Tree_DeleteElement(this, this->childs, info);
    ret_info = Tree_DeleteElement_v2(this, info);
